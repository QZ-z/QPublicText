...
undo log 是一种用于撤销回退的日志。在【事务没提交之前】，MySQL 会先记录【更新前的数据】到 undo log 日志文件里面，当事务回滚时，可以利用 undo log 来进行回滚
...
- 针对唯一索引进行检索时，对已存在的记录进行等值匹配时，将会自动优化为行锁（记录锁）
...
:warning:加锁都是针对索引项加，的每个索引项都指向数据行的实际位置，因此锁定一个索引项实际上就是锁定了一行数据。但如果没使用索引，InnoDB仍然会对行加锁，但这时锁定的是整个数据行，而不是索引记录。全表扫描实际上是给表中的所有记录行都加锁。
