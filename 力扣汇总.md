...
#### :warning:[网格中的最短路径](https://leetcode.cn/problems/shortest-path-in-a-grid-with-obstacles-elimination/)（多一维的bfs）
    ...
    public int shortestPath(int[][] grid, int k) {
        ...
        while(!que.isEmpty()){
            ...
            while(size-- >0){
                ...
                int curX = curState.x;
                int curY = curState.y;
                int curRest = curState.rest;
                for(int i = 0; i < 4; i++){
                    int nx = curX + dirs[i][0];
                    int ny = curY + dirs[i][1];
                    if(nx > - 1 && nx < m && ny > - 1 && ny < n){
                        if(grid[nx][ny] == 0 && !visited[nx][ny][curRest]){
                            if(nx == m - 1 && ny == n - 1) return step;
                            que.offer(new State(nx, ny, curRest));
                            visited[nx][ny][curRest] = true;
                        }else if(grid[nx][ny] == 1 && curRest > 0 && !visited[nx][ny][curRest - 1]){
                            que.offer(new State(nx, ny, curRest - 1));
                            visited[nx][ny][curRest - 1] = true;
                        }
                    }
                }
            }
...
#### 【多源bfs】[迷宫中离入口最近的出口](https://leetcode.cn/problems/nearest-exit-from-entrance-in-maze/)
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] dir : directions) {
                ...
                if (ni >= 0 && ni < maze.length && nj >= 0 && nj < maze[0].length 
                    && !visited.contains(ni + "," + nj) && maze[ni][nj] == '.') {
                    visited.add(ni + "," + nj);
                    queue.offer(new int[]{ni, nj, step + 1});
                }
            }
        }
